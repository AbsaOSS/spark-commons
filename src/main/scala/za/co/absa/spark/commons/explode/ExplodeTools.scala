...
object ExplodeTools {
  ...
  def explodeArray(arrayColPathName: String,
    ...
    val explodedColumnName = inputDf.schema.getClosestUniqueName(explosionTmpColumnName)
    ...
    val (dfWithTransientField, superTransientFieldName) = if (inputDf.schema.isOnlyField(arrayColPathName)) {
  ...
  def revertSingleExplosion(inputDf: DataFrame,
    ...
    val tmpColName = inputDf.schema.getClosestUniqueName(explosionTmpColumnName)
    ...
    val tmpColName2 = inputDf.schema.getClosestUniqueName(nullRestoredTmpColumnName)
  ...
  private def getRootLevelPrefix(fieldName: String, prefix: String, schema: StructType): String = {
    schema.getClosestUniqueName(s"${fieldName}_$prefix")
      .replaceAll("\\.", "_")
  }
