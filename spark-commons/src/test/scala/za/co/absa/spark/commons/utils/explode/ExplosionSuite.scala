...
class ExplosionSuite extends AnyFunSuite with SparkTestBase {
  ...
  test("Test handling of empty and null arrays") {
    ...
    assert(explodeContext.explosions.nonEmpty)
    assertSchema(explodedDf.schema, expectedExplodedSchema)
    assertResults(explodedDf, expectedExplodedResults, Seq("value_id")) // value_id is autogenerated, cannot be predicted, dropping from comparison
  ...
  private def assertResults(actualResults: DataFrame, expectedResults: DataFrame, ignoreColumns: Seq[String] = Seq.empty): Unit = {
    val ar = actualResults.drop(ignoreColumns: _*).collect()
    val er = expectedResults.drop(ignoreColumns: _*).collect()
