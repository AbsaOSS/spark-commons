...
trait ErrorHandling {
   ...
   * The purpose of this method is to convert the error specification into a `Column` expression
  ...
  /**
   * Applies the provided columns to the incoming DataFrame. Usually they might be aggregated in some way and attached
   * to the DataFrame, but any other operations are imaginable. Unless really bent, the incoming columns are those
   ...
   * @param dataFrame - the data frame to apply the error columns to
   ...
   * @param dataFrame - the data frame to operate on
  ...
  //TODO  Fix ScalaDoc cross-module links #48 - createErrorAsColumn(errorMessageSubmit: ErrorMessageSubmit)
  /**
   * Same as as above createErrorAsColumn(errorMessageSubmit: ErrorMessageSubmit), only providing the error specification
  ...
  /**
   * Applies the earlier collected [[types.ErrorColumn ErrorColumns]] to the provided DataFrame.
   * See [[doApplyErrorColumnsToDataFrame]] for detailed functional explanation
   * @param dataFrame - the data frame to operate on
   * @param errCols - a list of [[types.ErrorColumn]] returned by previous calls of `createErrorAsColumn`
   * @return - the original data frame with the error detection applied
   * @group Error Handling
   * @since 0.6.0
   */
